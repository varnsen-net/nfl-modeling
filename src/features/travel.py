...
def get_away_travel_distances(games, city_coords):
...
def make_travel_features(game, city_coords, path=TRAVEL_FEAT_PATH):
    """Build engineered features for team travel.

    :param pd.DataFrame game: raw games data
    :param pd.DataFrame city_coords: city coordinates
    :param str path: path to save travel features
    :return: None
    :rtype: None
    """
    away_travel_distances = get_away_travel_distances(game, city_coords)
